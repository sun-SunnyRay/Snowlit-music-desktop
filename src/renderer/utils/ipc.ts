...
export const getSourceAccountStatus = async() => {
  return rendererInvoke<LX.SourceAccount.Status[]>(WIN_MAIN_RENDERER_EVENT_NAME.source_account_status)
}

export const loginSourceAccount = async(id: LX.SourceAccount.Id) => {
  return rendererInvoke<LX.SourceAccount.Id, LX.SourceAccount.Status>(WIN_MAIN_RENDERER_EVENT_NAME.source_account_login, id)
}

export const logoutSourceAccount = async(id: LX.SourceAccount.Id) => {
  return rendererInvoke<LX.SourceAccount.Id, LX.SourceAccount.Status>(WIN_MAIN_RENDERER_EVENT_NAME.source_account_logout, id)
}

export const getSourceAccountPlaylists = async(id: LX.SourceAccount.Id) => {
  return rendererInvoke<LX.SourceAccount.Id, LX.SourceAccount.RemotePlaylist[]>(WIN_MAIN_RENDERER_EVENT_NAME.source_account_playlists, id)
}

export const getSourceAccountTracks = async(source: LX.SourceAccount.Id, id: string) => {
  return rendererInvoke<{ source: LX.SourceAccount.Id, id: string }, LX.Music.MusicInfoOnline[]>(WIN_MAIN_RENDERER_EVENT_NAME.source_account_tracks, { source, id })
}

export const saveAccountAutoSource = (source: LX.SourceAccount.Id) => {
  rendererSend(WIN_MAIN_RENDERER_EVENT_NAME.save_data, {
    path: DATA_KEYS.accountAutoSource,
    data: source,
  })
}

export const getAccountAutoSource = async() => {
