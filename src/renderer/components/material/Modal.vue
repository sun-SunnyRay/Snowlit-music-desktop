...
export default {
  ...
  data() {
    return {
      ...
      //   'hinge',
      // ],
      // 默认用快进场，更跟手；开启「随机动画」时再换成花哨效果
      inClass: 'animated-fast zoomIn',
      outClass: 'animated-fast fadeOut',
      showModal: false,
      showContent: false,
      modalCount: false,
      isAddedClass: false,
  ...
  methods: {
    ...
    setRandomAnimation() {
      if (appSetting['common.randomAnimate']) {
        ...
        this.inClass = 'animated-fast zoomIn'
        this.outClass = 'animated-fast fadeOut'
      }
    },
    close() {
      this.$emit('close')
