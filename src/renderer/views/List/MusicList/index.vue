<template>
  <div :class="$style.list">
    <div v-if="showAccountSourceBar" :class="$style.sourceBar">
      <button
        v-for="id in accountLoggedIn"
        :key="id"
        :class="[$style.sourceBtn, { [$style.sourceActive]: accountSource == id }]"
        :disabled="accountLoading"
        @click="handleAccountSource(id)"
      >{{ $t('account_playlist__' + id) }}</button>
    </div>
    <div class="thead">
      <table>
        <thead>
    ...
    <div v-show="!list.length" :class="$style.noItem">
      <p v-if="accountError == 'EMPTY'" v-text="$t('account_auto__empty')" />
